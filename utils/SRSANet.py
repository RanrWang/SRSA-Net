...
class _SRSANetResidualUnit(nn.Module):
    """
    Residual unit.
    ...
    def __init__(self, input_channels, output_channels, stride):
        super(_SRSANetResidualUnit, self).__init__()
        ...
        self.depth_conv = nn.Conv2d(in_channels=internal_channels,
                                    out_channels=internal_channels,
                                    kernel_size=3,
                                    stride=1,
                                    padding=1,
                                    groups=internal_channels)
        self.point_conv = nn.Conv2d(in_channels=internal_channels,
                                    out_channels=internal_channels,
                                    kernel_size=1,
                                    stride=1,
                                    padding=0,
...
def _make_SRSANet_residual_block(input_channels, output_channels, stride, n_units):
    """
    ...
    units.append(_SRSANetResidualUnit(input_channels, output_channels, stride))

    for i in range(n_units - 1):
        units.append(_SRSANetResidualUnit(output_channels, output_channels, stride=1))
...
class _SRSANetEncoder(nn.Module):
    """
...
'''the prior version of decoder'''
# class _SRSANetDecoder(nn.Module):
#     """
#     One of the three identical decoder branches.
#     """
#
#     def __init__(self):
#         super(_SRSANetDecoder, self).__init__()
...
'''the attention gate'''
# class _Attention_decoder_Unit(nn.Module):
#
#     def __init__(self, input_channels,output_channels):
#         super(_Attention_decoder_Unit, self).__init__()
#         internal_channels = input_channels
#         self.conv1=nn.Conv2d(input_channels,internal_channels,kernel_size=1)
#         self.conv2=nn.Conv2d(internal_channels,internal_channels,kernel_size=3,padding=1)
#         self.conv3 = nn.Conv2d(input_channels, internal_channels, kernel_size=3,padding=1)
#         self.upsample = nn.Upsample(scale_factor=2)
#         self.relu = nn.ReLU()
#         self.conv4 = nn.Conv2d(internal_channels,internal_channels, kernel_size=3, padding=1)
#         self.sigmoid = nn.Sigmoid()
#         self.pool = nn.MaxPool2d(2,2)
#         self.conv6 = nn.Conv2d(internal_channels, output_channels, kernel_size=1)
#         self.bnrelu6 = _BatchNormRelu(output_channels)
#
#     def forward(self, input1,input2):
#         out1 = self.conv1(input1)
#         out1 = self.conv2(out1)
#         out1 = self.upsample(out1)
#         out2 = self.conv3(input2)
...
#         out = torch.multiply(out,input2)
#         out = self.conv6(out)
#         out = self.bnrelu6(out)
#         return out

'''self attention in Self-Attention GAN'''
...
'''self attention mechanis in Polarized Self-Attention'''
...
#         self.attention1 = Channel_only_branch(channel)
#         self.attention2 = Spatial_only_branch(channel)
#     def forward(self,x):
...
'''self attention mechanism in CCNet(CrissCrossAttention)'''
# def INF(B, H, W):
...
'''the version of attention-gate model4 decoder'''
# class _SRSANetDecoder(nn.Module):
#     """
#     One of the three identical decoder branches.
#     """
...
#         super(_SRSANetDecoder, self).__init__()
#         self.upsample1 = nn.Upsample(scale_factor=2)
#         # self.Self_Attn1 = Self_Attn(1024, nn.ReLU())  #self_attention GAN
#         # self.CCattention = CrissCrossAttention(in_dim=1024)    #self_attention CCNet
#         # self.PSattention = attention(in_dim=1024)     #Polarized Self-Attention
#         # self.conv1 = nn.Conv2d(
#         #     in_channels=1024,
#         #     out_channels=256,
#         #     kernel_size=5,
...
#         #     bias=False,
#         # )
#         self.attention1 = _Attention_decoder_Unit(input_channels=1024, output_channels=256)   #attention gate
#         self.dense1 = _make_HoVerNet_dense_block(input_channels=256, n_units=8)
#         self.conv2 = nn.Conv2d(
#             in_channels=512, out_channels=512, kernel_size=1, stride=1, bias=False
#         )
...
#         #     bias=False,
#         # )
#         self.attention2 = _Attention_decoder_Unit(input_channels=512, output_channels=128)
#         self.dense2 = _make_HoVerNet_dense_block(input_channels=128, n_units=4)
#
#         self.conv4 = nn.Conv2d(
#             in_channels=256, out_channels=256, kernel_size=1, stride=1, bias=False
...
#         # skip connection addition
#         # out = out + block3_out
#         # out = self.Self_Attn1(out) #self_attention GAN
#         # out = self.CCattention(out) #CCNet
#         # out = self.PSattention(out) #PSA
#         # out = self.conv1(out)
#         out = self.attention1(out, block3_out)  #attention gate
#         out = self.dense1(out)
#         out = self.conv2(out)
...
#         # out = out + block2_out
#         # out = self.conv3(out)
#         out = self.attention2(out, block2_out)
#         out = self.dense2(out)
#         out = self.conv4(out)
#         out = self.upsample1(out)
#         # last skip connection
...
'''the version of self-attention decoder'''
class _SRSANetDecoder(nn.Module):
    """
    One of the three identical decoder branches.
    """
...
#         super(_SRSANetDecoder, self).__init__()
#         self.upsample1 = nn.Upsample(scale_factor=2)
#         self.Self_Attn1 = Self_Attn(256, nn.ReLU())  #self_attention GAN
#         # self.attention = CrissCrossAttention(in_dim=1024)    #self_attention CCNet
#         # self.attention = attention(1024)     #Polarized Self-Attention
#         self.conv1 = nn.Conv2d(
#             in_channels=1024,
#             out_channels=256,
#             kernel_size=5,
...
#             in_channels=512, out_channels=512, kernel_size=1, stride=1, bias=False
#         )
#         # self.Self_Attn2 = Self_Attn(512, nn.ReLU())
#         self.conv3 = nn.Conv2d(
#             in_channels=512,
#             out_channels=128,
#             kernel_size=5,
...
#         # skip connection addition
#         out = out + block3_out
#         # out = self.attention(out) #CCNet
#         # out = self.attention(out) #PSA
#         out = self.conv1(out)
#         # out = self.attention1(out, block3_out)  #attention gate
#         out = self.Self_Attn1(out) #self_attention GAN
#         out = self.dense1(out)
...
class SRSANet(nn.Module):
    ...
    def __init__(self, n_classes=None):
        ...
        self.encoder = _SRSANetEncoder()

        # NP branch (nuclear pixel)
        self.np_branch = _SRSANetDecoder()
        # classification head
        ...
        self.hv_branch = _SRSANetDecoder()  # hv = horizontal vertical
        # classification head
        ...
        if self.n_classes is not None:
            self.nc_branch = _SRSANetDecoder()
            # classification head
...
def post_process_batch_hovernet(
    ...
    for i in range(batchsize):
        ...
        ################################### no watershed processing###################
        "for comparing the performance of post_process, comment out the line of preds = _post_process_single_hovernet" \
        ", and uncomment the following"
        # np_preds = F.softmax(np_out[i,...], dim=0)[1, :, :]
        # np_preds = np_preds.numpy()
        # np_preds[np_preds >= h] = 1
        # np_preds[np_preds < h] = 0
        # np_preds = np_preds.astype(np.uint8)
