...
def multiclass_mask_to_nucleus_mask(multiclass_mask):
    """
    ...
    assert (
        multiclass_mask.ndim == 3 and multiclass_mask.shape[0] == 3
    ), f"Expecting a mask with dims (3, 256, 256). Got input of shape {multiclass_mask.shape}"
    assert (
